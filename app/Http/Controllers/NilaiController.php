   ...
   public function index()
{
    ...
    if ($user->role === 'guru') {
        ...
        // Guru biasa: semua mapel kecuali PAI dan PAI 2
        $mapel = Mapel::whereNotIn('nama_mapel', ['PAI', 'PAI 2'])->get();
        ...
        // Guru PAI: hanya mapel PAI dan PAI 2
        $mapel = Mapel::whereIn('nama_mapel', ['PAI', 'PAI 2'])->get();

    } elseif ($user->role === 'siswa') {
        $siswa = Siswa::where('id', $user->siswa->id)->with('nilai.mapel', 'kelas')->get();
        $mapel = Mapel::all(); // Atau kosongkan jika siswa tidak bisa input
    } else {
        // Role lain (admin, dll)
